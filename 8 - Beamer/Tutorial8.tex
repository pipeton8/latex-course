% En esta parte crearemos nuestra primera presentación en Beamer

\documentclass[11pt]{beamer}	% Aquí llamamos a la clase beamer. Siempre podríamos crear nuestra propia clase, con opciones y otras cosas para hacerlo más inteligente.
...
% Beamer tiene precargado el paquete hyperref, por lo que incluiremos las opciones con el comando ``hypersetup''
...
% Alternativamente, pueden buscar en internet otros temas e instalarlos manualmente.

% Para definir el tema, el color y el tipo de letra, usamos la siguiente sintaxis
\usetheme{Madrid}	
...
% La unidad básica de beamer es el ``frame''. Cada nuevo frame se llama usando la siguiente sintaxis:
%	\begin{frame}[Opciones]
%		\frametitle{Título del frame}	
%		Material del frame
%	\end{frame}
...
% Vale decir que no siempre un ``frame'' es una página del PDF que sale, pero eso lo veremos más adelante.
...
% A continuación veremos uno de los entornos más usados en Beamer, el entorno Block que, como su nombre lo dice, encierra en ``bloques'' el texto
\begin{frame}
	\frametitle{\secname}
	...
	\end{block} \vfill % el uso de \vfill ayuda a que la diapositiva se distribuya de mejor manera en dirección vertical. Si no nos gusta cómo queda podemos usar \vspace{medida} donde medida es una medida con unidades, como por ejemplo ``1cm''.
...
\begin{frame}
	...
	% El comando más simple para animar es ``pause'', el cual genera una diapositiva adicional por cada vez que aparece y que corta la diapositiva justo en donde se le llama. En el PDF lo que veremos son distintas páginas, que al avanzar entregan la ilusión de animar la presentación.
...
\begin{frame}
	...
	Pero también es posible que los demás elementos \alert{sí} se vean, usando el comando \texttt{\textbackslash setbeamercovered\{Opción\}}. Este puede llamarse en el preámbulo (si queremos un comportamiento general) o en algún frame en particular (si queremos un comportamiento local). Las opciones son:
		...
		% A mí me gusta usar \smallskip después de cada elemento de itemize para que la distribución vertical sea más bonita.
\end{frame}
